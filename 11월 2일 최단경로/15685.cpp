...
using namespace std;
typedef pair<int, int> ci; //좌표를 pair로 저장
    ...
    for (int i = 0; i < SIZE - 1; i++) {
        for (int j = 0; j < SIZE - 1; j++) {
            if (board[i][j] && board[i][j + 1] && board[i + 1][j] && board[i + 1][j + 1])
                ans++;
            //(i,j) 기준 (i,j), (i,j+1), (i+1,j), (i+1, j+1)이 모두 true인지(방문되었는지)확인하고 ans증가
        }
    }
    return ans; //네 꼭짓점이 모두 드래곤 커브의 일부인 정사각형 개수 리턴
}

ci makeDragonCurve(vector<vector<bool>> &board, vector<int> &direc, ci point) {
    int size = direc.size(); //그 전 세대까지의 방향 정보
    for (int i = size - 1; i >= 0; i--) {
        int new_direc = (direc[i] + 1) % 4; //현재 시작 점의 방향-모듈러 연산으로 계산하기(방향이 0~3이므로)
        point = {point.first + dy[new_direc], point.second + dx[new_direc]}; //현재 끝 점
        board[point.first][point.second] = true; //현재 끝 점 방문 표시
        direc.push_back(new_direc); //계산한 new_direc을 direc에 저장 
    }
    return point; //for문 다 돌고 끝 점 리턴
}
